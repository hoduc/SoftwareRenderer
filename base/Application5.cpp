...
GzRender aarender[AAKERNEL_SIZE];
...
int copyRender(GzRender* src, GzRender* dest)
{
	int result = GZ_SUCCESS;
	if (src == NULL || dest == NULL)
		return GZ_FAILURE;
	memcpy(dest, src, sizeof(GzRender));
	dest->display = NULL; //dont copy display
	return result;
}
//////////////////////////////////////////////////////////////////////
...
int Application5::Initialize()
{
	...
	GzToken     nameSample[AAKERNEL_SIZE];
	GzPointer   valueSample[AAKERNEL_SIZE];
...
*/
	...
	*/
	...
	{
		copyRender(m_pRender, &aarender[i]);
		GzNewDisplay(&aarender[i].display, xRes, yRes);
		aarender[i].xOffset = AAFilter[i][0];
		aarender[i].yOffset = AAFilter[i][1];
		aarender[i].weight = AAFilter[i][2];
	}
	if (status) 
		return(GZ_FAILURE); 
	else 
...
int Application5::Render() 
{
	...
	*/ 
	...
	for ( int i = 0; i < AAKERNEL_SIZE; i++ )
		GzInitDisplay(aarender[i].display);
	/* 
	...
	*/ 
	while( fscanf(infile, "%s", dummy) == 1) { 	/* read in tri word */
	     ...
	     */ 
		 ...
		 for (int i = 0; i < AAKERNEL_SIZE; i++)
		 {
			 GzPutTriangle(&aarender[i], 3, nameListTriangle, valueListTriangle);
		 }
	...
	{
		GzPixel* pixel = &m_pDisplay->fbuf[i];
		float red = 0;
		float green = 0;
		float blue = 0;
		...
		{
			GzPixel* aaPixel = &aarender[j].display->fbuf[i];
			float weight = aarender[j].weight;
			red += aaPixel->red*weight;
			green += aaPixel->green*weight;
			blue += aaPixel->blue*weight;
			//pixel->alpha += aaPixel->alpha*weight;
		}
		pixel->red = red;
		pixel->green = green;
		pixel->blue = blue;
...
int Application5::Clean()
{
	...
	for (int i = 0; i < AAKERNEL_SIZE; i++)
	{
		status |= GzFreeDisplay(aarender[i].display);
	}
	if (status) 
		return(GZ_FAILURE); 
	else 
